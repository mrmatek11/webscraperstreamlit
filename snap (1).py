...
KLUCZOWE ZMIANY vs v3:
  ...
  #B  Architektura: ONE browser per run, context per task.
      v3 startował Chromium per page (300+ MB, ~1-2s) — v4 robi to raz.
...
#
# Działa na poziomie context (czyli dla każdej nowej page). Robi 4 rzeczy
# zanim strona w ogóle ruszy:
#   1) globalne CSS-y wyłączające animacje/transition (override styles strony)
#   2) IntersectionObserver hijack — od razu wywołuje callback z isIntersecting=true
#      Dzięki temu WOW.js, AOS, Webwave Observer, Elementor, lozad — wszystko
#      myśli że każdy element jest "w viewport" od początku.
#   3) Pre-set localStorage i sessionStorage z flagami konsensu cookies
#      (dla Webwave: cookieSettings, dla Cookiebot, OneTrust itd.)
#   4) Override window.matchMedia('prefers-reduced-motion') → 'reduce'
...
(() => {
    ...
    // Trick: gdy strona robi `new IntersectionObserver(cb)` to zwracamy
    // nasz proxy który natychmiast po observe(el) odpala callback z
    // isIntersecting=true. Tym samym wszystkie biblioteki które
    // "czekają aż element wejdzie w viewport" myślą że już wszedł.
    try {
        ...
        if (RealIO) {
            window.IntersectionObserver = class FakeIO {
                ...
                observe(el) {
                    ...
                    // odpalmy zarówno natychmiast jak i z opóźnieniem (na
                    // wypadek że strona dodaje element po observe)
                    setTimeout(fire, 0);
                    setTimeout(fire, 200);
                }
    ...
    // (context.reduced_motion='reduce' też to ustawia, ale ten override
    //  działa też dla starych media queries które niektóre strony cachują)
    try {
...
def url_to_local_path(asset_url: str, assets_dir: Path, fname_counts: dict, content_type: str = ""):
    ...
    # split query string PRZED sanitize żeby '?' nie został zastąpiony przez '_'
...
# ─── popup killing (zachowane z v3, leciutko rozszerzone) ─────────────────────
...
def close_popups(page):
    # Rozszerzone selektory: "Allow All", "Akceptuj wszystkie", "Zezwól na wszystkie"
    dismiss_selectors = [
        ...
        # Klikam buttony po tekście (przez page.get_by_text obsłużę osobno)
    ...
    # Klik po tekście — łapie "Allow All", "Akceptuj wszystkie" itd.
    accept_texts = [
...
def _force_lazy_load(page):
    """Rozszerzona wersja v3: obsługuje Webwave (data-lazy-load-src,
    data-lazy-background-image, data-ww_rwd) + Lozad (data-background-image).
    Pokrywa większość polskich i światowych frameworków lazy."""
    try:
        page.evaluate(r"""
            () => {
                const SRC_ATTRS = [
                    'data-lazy-src', 'data-src', 'data-original',
                    'data-lazy-load-src',   // Webwave
                    'data-img-src',
                    'data-echo',            // echo.js
                    'data-srcdefer',
                ];
                ...
                const BG_ATTRS = [
                    ...
                    'data-background-image',     // Lozad/Slick
                    'data-lazy-background-image',// Webwave
                    'data-background',
                ];
                ...
                // ── Webwave: data-ww_rwd JSON parsing ──────────────
                // Format: {"data-lazy-load-src":{"rwdMode_1":"..."}, ...}
                ...
                // ── Webwave: <figure> w sliderze ma <link itemprop="contentUrl">
                //    + <script type="application/json"> z rwdProperties
                document.querySelectorAll('figure script[type="application/json"]').forEach(s => {
                    try {
                        ...
                        if (data.rwdProperties) {
                            ...
                            if (m1 && m1.imageSrc) {
                                // znajdź <img> w tym samym slajdzie
...
def _finish_all_animations(page):
    """Jedno wywołanie zamiast 5-6 starych _force_*_render. Łączy:
       - Web Animations API: document.getAnimations().finish()
       - GSAP: globalTimeline.progress(1)
       - anime.js: anime.running[i].seek(duration)
       - WOW.js: ręcznie dodaj klasę 'animated' + opacity:1
       - AOS: dodaj 'aos-animate'
       - SR7 / RevSlider — zostawiam stare funkcje (specyficzne)
    """
    try:
        page.evaluate("""
            () => {
                // Web Animations API — natywne CSS+WAAPI animacje
                try {
                ...
                // WOW.js / animate.css — kluczowe dla lawendowyzdroj.pl
                ...
                // Video — pauza + lekki seek żeby pokazać klatkę
...
def _walk_through_sliders(page):
    """Przechodzi przez wszystkie slajdy żeby wymusić załadowanie obrazów
    w tych co nie są aktualnie widoczne. Obsługuje:
      - Slick (.slick-dots li, .slick-next)
      - Flickity (.flickity-page-dots .dot)
      - Owl (.owl-dot)
      - Webwave gallery (.gv_panel, .gv_panelNavNext)
      - Swiper (.swiper-pagination-bullet)
    Robi to za pomocą JS clicków (nie page.click() — szybciej, mniej crashów)."""
    try:
        page.evaluate("""
            () => {
                const dotSelectors = [
                ...
                // Webwave gallery — klik na panel-next
                ...
                // Klikaj next-buttony agresywnie
                const nextSelectors = [
...
def _wait_for_images(page, timeout=5000):
    """v3 sprawdzał tylko img.complete — to zwraca true też dla 404.
    v4 sprawdza naturalWidth > 0."""
    try:
...
# ─── SR7 / RevSlider (zachowane z v3 — specyficzne dla tych pluginów) ─────────
...
def _force_sr7_render(page):
    ...
    # Fallback — wyciągnij obrazy z noscript
    try:
        page.evaluate("""
            () => {
...
def _navigate(page, url: str, retries: int = 2) -> Tuple[bool, str]:
    for attempt in range(1 + retries):
        try:
            # 'load' zamiast 'networkidle' — networkidle hangs na stronach z analytics polling
            page.goto(url, wait_until='load', timeout=45000)
            # potem opcjonalne czekanie na networkidle z krótszym timeoutem
            try:
...
# ─── asset capture & html rewriting (z v3, drobne fixy) ───────────────────────
...
def _prepare_page(page, aggressive: bool):
    """Wspólna logika dla full i screenshot: po _navigate woła tę funkcję
    która ogarnia popups + lazy + animations + sliders. Wynik to gotowa
    do screenshotu strona."""
    # 1) zabij popupy zanim cokolwiek innego
    close_popups_aggressive(page) if aggressive else close_popups(page)

    # 2) scroll przez całą stronę żeby IntersectionObserver-hijack zadziałał
    _scroll_and_wait(page)

    # 3) Lazy loading — rozszerzony
    _force_lazy_load(page)

    # 4) Specyficzne slidery (SR7, RevSlider)
    _force_sr7_render(page)
    _force_revslider_render(page)

    # 5) Slider walkthrough — klikaj dots
    _walk_through_sliders(page)

    # 6) jeszcze raz lazy bo slider mogł odsłonić nowe obrazy
    _force_lazy_load(page)

    # 7) Pop-upy które pojawiły się PO scrolu (exit-intent itp)
    close_popups(page)

    # 8) Animacje — wszystko do końca
    _finish_all_animations(page)

    # 9) Czekaj na obrazy + fonty
    _wait_for_images(page, timeout=8000)
    _wait_for_fonts(page)

    # 10) Końcowe utrzymanie
    try:
    ...
    # 11) Jeszcze raz animacje (na wypadek scroll-triggered)
    _finish_all_animations(page)
...
def process_full(page_url: str, context, output_dir: Path, aggressive: bool = False,
    ...
    # Pobierz brakujące pliki przez requests fallback
    ...
    # Złap dodatkowe obrazy — w tym te z data-ww_rwd których nie pobierano
    try:
    ...
    try:
        ...
        # Re-rewrite po dociągnięciu CSS-ów
        try:
            html = _rewrite_html(html, captured, page_url)
            (output_dir / 'index.html').write_text(html, encoding='utf-8', errors='replace')
        except Exception:
...
# ─── #B: nowy model — jeden browser, wiele context ────────────────────────────

def _make_context(browser):
    """Tworzy nowy context z wszystkimi init scriptami. Wywoływane per task."""
...
def run(urls: list, base_output: Path, mode: str, keep_folders: bool = False):
    ...
    # ── jeden Playwright + Chromium na cały run ─────────────────────────────
    ...
    try:
        _run_inner(normalized, base_output, mode, keep_folders, browser)
    finally:
        try: browser.close()
...
def _run_inner(normalized, base_output, mode, keep_folders, browser):
